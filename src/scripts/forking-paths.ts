 ...
 */
import { mulberry32, seedFromUrl } from '../lib/rng';
...
const xOf = (p: number) => x0 + (1 - Math.pow(p, AXIS_POWER)) * (x1 - x0);
...
function colourOf(p: number): string {
  if (p > THRESHOLD) {
...
function drawField(paths: Path[]) {
  while (layerField.firstChild) layerField.removeChild(layerField.firstChild);
  ...
  for (const it of items) {
    ...
    layerField.appendChild(c);
    dots.push({ c, sig: it.sig, cx: it.cx });
  }

  if (reduced()) {
    layerField.querySelectorAll('.halo').forEach((h) => h.setAttribute('opacity', '.45'));
    dots.forEach((d) => d.c.setAttribute('opacity', d.sig ? '1' : '.72'));
  } else {
    const order = dots.slice().sort((a, b) => a.cx - b.cx);
    order.forEach((d, i) =>
      setTimeout(() => d.c.setAttribute('opacity', d.sig ? '1' : '.72'), i * 3),
    );
    setTimeout(
      () => layerField.querySelectorAll('.halo').forEach((h) => h.setAttribute('opacity', '.45')),
      order.length * 3 + 100,
